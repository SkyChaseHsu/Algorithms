def paint(filename, title):
	...
	for data_name in data_names:
		if data_name.find('THE_') == -1:
			...
			if data_name in ['BUBSORT', 'SELSORT', 'INSSORT']:
...
def getTheory(filename):
	...
	sort_name_01 = ['MERSORT', 'QUISORT']
	sort_name_02 = ['SELSORT', 'BUBSORT', 'INSSORT']
		...
		for index, values in enumerate(reader):
			...
			if values[0] in sort_name_01:
				if values[1] == '10000':
					...
					the_data.append(['THE_WOR_'+values[0], values[1], genTime(base_time, int(values[1]), 1),])
					the_data.append(['THE_BES_'+values[0], values[1], genTime(base_time, int(values[1]), 1) / 2,])
			if values[0] in sort_name_02:
				if values[1] == '10000':
					base_time = float(values[2])
					the_data.append(['THE_'+values[0], values[1], values[2],])
				else:
					the_data.append(['THE_'+values[0], values[1], genTime(base_time, int(values[1]), 2),])
...
def genTime(in_time, N, gen_type):
	import math
	if gen_type == 1:
		per_time = in_time / (10000 * math.log10(10000))
		out_time = per_time * N * math.log10(N)
		return round(out_time, 4)
	elif gen_type == 2:		
		per_time = in_time / pow(10000, 2)
		out_time = per_time * pow(N, 2)
		return round(out_time,4)
		

# getTheory('expriData01.csv')
# getTheory('expriData02.csv')

paint('expriData01.csv', 'Time efficientcy of sort algorithm')
paint('expriData02.csv', 'Time efficientcy of sort algorithm')